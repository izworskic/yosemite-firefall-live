    ...
    return <>
      <section className="hero preseason" aria-labelledby="hero-title">
        ...
        <div className="hero-content">
          ...
          <h1 id="hero-title">Horsetail Fall <span>2027</span></h1>
          ...
          <div className="preseason-grid">
            ...
            <div><b>2027 rules pending</b><span>Prior-year access rules are not assumed</span></div>
          </div>
        </div>
      </section>
      <section className="section tight">
        <div className="section-heading"><div><p className="eyebrow dark">SOLAR ALIGNMENT</p><h2>First look at 2027 geometry</h2></div><p className="muted">These cards are geometry only, not weather probabilities.</p></div>
    ...
    <section className="hero" aria-labelledby="hero-title">
      ...
      <div className="hero-content">
        ...
        <div className="metric-grid">
          ...
          <Metric label="Western sky" value={selected.cloudOpen === null ? 'Unknown' : `${selected.cloudOpen}% open`} detail="event-window cloud model"/>
          <Metric label="Sun geometry" value={`${selected.geometry}/100`} detail="solar alignment"/>
...
function SourceStrip({ snapshot }: { snapshot: FirefallSnapshot }) {
  return <section className="section sources"><div className="section-heading"><div><p className="eyebrow dark">DATA HEALTH</p><h2>What the engine can see</h2></div><p className="muted">Generated {new Date(snapshot.generatedAt).toLocaleString()}</p></div><div className="source-grid">{snapshot.sources.map(s => <div className="source" key={s.source}><span className={`dot ${s.freshness}`}/><div><b>{s.source}</b><small>{s.freshness}{s.note ? ` · ${s.note}` : ''}</small></div></div>)}</div></section>;
}
